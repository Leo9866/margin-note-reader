...
- 工作区首页：可以新建空白文档、打开 Markdown、打开文件夹、打开 HTML。
...
- PPTX 导出：把当前 Markdown / HTML 文档按企业方案风格自动生成 PowerPoint。
...
- 选择一个本地文件夹并批量导入其中的 Markdown、纯文本和 HTML 文件。
- 打开 `.html` / `.htm` 文件，应用会提取正文结构、图片、内联 SVG 图示、链接等内容并转换为 Markdown 阅读。

用户新建和导入的文档可以点击顶部 **编辑源文** 进入 Markdown 源文编辑模式；保存后会重新生成目录和阅读块，并写回 IndexedDB。

打开文档后可以点击顶部 **导出 PPT**，应用会把当前文档切分为封面、目录、章节页、要点页和图片页，并下载 `.pptx` 文件。

用户文档旁的删除操作只会移除浏览器本地文档库里的记录，以及该文档关联的本地笔记和 AI 沉淀；不会删除电脑上的原始文件。

如果 HTML 或 Markdown 使用相对路径引用本地图片，建议通过 **打开文件夹** 导入，这样应用可以同时读取图片资源并内嵌到本地文档库中。HTML 中直接写在页面里的 SVG 图示会自动转成内嵌图片保存。

项目默认不会加载任何内置资料，适合作为空白阅读工作台开始。你可以通过工作区首页导入自己的 Markdown、纯文本或 HTML 文件。
